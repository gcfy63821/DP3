...
class DataCollector:
    def __init__(self):
        ...
        self.max_length = 10  # 设置 deque 的最大长度
        self.ft_compensated_data = deque(maxlen=self.max_length)
        self.netft_data = deque(maxlen=self.max_length)
        self.agent_pos_data = deque(maxlen=self.max_length)
        self.img_data = deque(maxlen=self.max_length)
        self.pointcloud_data = deque(maxlen=self.max_length)
    ...
    def collect_data(self):
        ...
        while not rospy.is_shutdown():
            ...
            synced_pointcloud = self.get_closest_data(self.pointcloud_data, current_time)

            data_dict = {}

            if synced_ft_compensated and synced_agent_pos and synced_img and synced_pointcloud:
                ft, ft_timestamp = synced_ft_compensated
                agent_pos, agent_pos_timestamp = synced_agent_pos
                frame, img_timestamp = synced_img
                point_cloud, pc_timestamp = synced_pointcloud
                ...
                data_dict['point_cloud'] = point_cloud # 点云
                ...
                if not synced_pointcloud:
                    print("No pointcloud synced")
