...
save_data_path = expert_data_path + '/data_for_pretrain/'
save_data_path_this_episode = [os.path.join(save_data_path, f) for f in os.listdir(expert_data_path) if os.path.isdir(os.path.join(expert_data_path, f))]
print(save_data_path_this_episode)
save_data_path_this_episode = sorted(save_data_path_this_episode)
os.mkdir(save_data_path)
for folder in save_data_path_this_episode:
    os.mkdir(folder)
...
for j,subfolder in enumerate(subfolders):
    ...
    for k, npy_file in enumerate(npy_files):
        ...
        # ============================ add a state for pretrain
        data_dict["task_state"] = task_state
        ...
        new_npy_file = os.path.join(save_data_path_this_episode[j], os.path.basename(npy_file))
        np.save(new_npy_file, data_dict)
        ...
        # 按 'q' 键退出，按回车键增加 task_state
        key = cv2.waitKey(0)  # 等待按键按下
        if key == ord('q'):
            break
        elif key == 13:  # 回车键的 ASCII 码是 13
            task_state += 1
            print(f"task_state increased to {task_state}")
