...
class UltrasoundPCDataset(BaseDataset):
    def __init__(self,
            zarr_path, 
            horizon=1,
            pad_before=0,
        ...
        self.task_name = task_name
        self.replay_buffer = ReplayBuffer.copy_from_path(
            zarr_path, keys=['state', 'action', 'force', 'img', 'point_cloud'])
        val_mask = get_val_mask(
            n_episodes=self.replay_buffer.n_episodes, 
            val_ratio=val_ratio,
            seed=seed)
...
class UltrasoundForcePositionDataset(BaseDataset):
    def __init__(self,
            zarr_path, 
            horizon=1,
            pad_before=0,
        ...
        self.replay_buffer = ReplayBuffer.copy_from_path(
            zarr_path, keys=['state', 'action', 'force'])
        val_mask = get_val_mask(
            n_episodes=self.replay_buffer.n_episodes, 
            val_ratio=val_ratio,
            seed=seed)
...
class UltrasoundForcePositionDataset1(BaseDataset):
    def __init__(self,
            zarr_path, 
            horizon=1,
            pad_before=0,
        ...
        super().__init__()
        self.task_name = task_name
        zarr_path = 'data/ultrasound_data_force_position1.zarr'
        self.replay_buffer = ReplayBuffer.copy_from_path(
            zarr_path, keys=['state', 'action', 'force'])
        val_mask = get_val_mask(
            n_episodes=self.replay_buffer.n_episodes, 
...
class Ultrasound2CamDataset(BaseDataset):
    def __init__(self,
            zarr_path, 
            horizon=1,
            pad_before=0,
    ...
    def _sample_to_data(self, sample):
        ...
        image = sample['img'][:,].astype(np.float32)
        force = sample['force'][:,].astype(np.float32)
        image2 = sample['img2'][:,].astype(np.float32)
