...
from diffusion_policy_3d.model.vision.my_encoder import UltrasoundDPEncoder, USPCEncoder, ForcePositionEncoder, Ultrasound2CamEncoder
...
class USPCDP(BasePolicy):
    ...
    def conditional_sample(self, 
        ...
        # print(f"Trajectory is on device: {trajectory.device}")
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if self.obs_as_global_cond:
            ...
            nobs_features = self.obs_encoder(this_nobs)
            # time_2 = time.time()
            # print('encoding time:', time_2-time_1)
            if "cross_attention" in self.condition_type:
                # treat as a sequence
                global_cond = nobs_features.reshape(B, self.n_obs_steps, -1)
            else:
        ...
        nsample = self.conditional_sample(
            ...
            **self.kwargs)
        time_4 = time.time()
        print("conditional sample time:", time_4 - time_3)
...
class USForcePositionDP(BasePolicy):
    def __init__(self, 
            shape_meta: dict,
            noise_scheduler: DDPMScheduler,
            horizon, 
        ...
        obs_encoder = ForcePositionEncoder(  
                                            observation_space=obs_dict,
                                            out_channel=encoder_output_dim,
                                            ultrasound_encoder_cfg = ultrasound_encoder_cfg,
                                        )
...
class Ultrasound2CamDP(BasePolicy):
    def __init__(self, 
            shape_meta: dict,
            noise_scheduler: DDPMScheduler,
            horizon, 
        ...
        obs_encoder = Ultrasound2CamEncoder(  
                                            observation_space=obs_dict,
                                            out_channel=encoder_output_dim,
                                            ultrasound_encoder_cfg = ultrasound_encoder_cfg,
                                        )
