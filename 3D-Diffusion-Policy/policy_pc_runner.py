...
class PolicyROSRunner:
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        self.pointcloud_queue = deque(maxlen=self.n_obs_steps)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # if synced_agent_pos:
                ...
                self.pointcloud_queue.append(point_cloud)

                # 确保队列长度与 n_obs_steps 匹配
                if len(self.state_queue) < self.n_obs_steps:
                    continue  # 如果队列长度不足，等待更多数据

                state_array = np.array(list(self.state_queue))
                force_array = np.array(list(self.force_queue))
                img_array = np.array(list(self.img_queue))
                ...
                # pointcloud_tensor = torch.tensor(point_cloud, dtype=torch.float32, requires_grad=False).unsqueeze(0).unsqueeze(0).to(self.device)

                obs_dict = {
                    "state": state_tensor,
                    "force": force_tensor,
                    "img": img_tensor,
                    "point_cloud": pointcloud_tensor,
                }
